document.addEventListener('DOMContentLoaded', () => {
	...
	/* 初始化首页列表功能 */
	{
		const getListMode = _ => {
            if(_.type != 'sticky') {
		        var s1 = new Date(_.time.replace(/-/g, "/"));
                var s2 = new Date();
                var days = s2.getTime() - s1.getTime();
                var time = parseInt(days / (1000 * 60 * 60 * 24));
                if(window.Joe.ShowNewArticle != '' && window.Joe.ShowNewArticle != 'off') {
                    if(time <= window.Joe.ShowNewArticle) {
                        _.isNew = 'isNew';
                    }
                }
		    }
                    ...
                    <li class="joe_list__item wow default">
                        ...
                        <div class="information">
                            <a href="${_.permalink}" class="title" title="${_.title}" target="_blank" rel="noopener noreferrer">
                                ...
                                <span class="badge" style="display: ${_.isNew === 'isNew' ? 'inline-block' : 'none'}">NEW</span>
                                ${_.title}
                            </a>
		...
		let queryData = { page: 1, pageSize: window.Joe.PAGE_SIZE, type: 'modified' };
		const initDom = () => {
